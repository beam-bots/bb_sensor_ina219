...
defmodule Mix.Tasks.BbIna219.InstallTest do
  use ExUnit.Case
  import Igniter.Test
  ...
  describe "parameters group" do
    test "adds a :config.:ina219 param group with bus and address" do
      project_with_robot()
      |> Igniter.compose_task("bb_ina219.install")
  ...
  describe "application module" do
    ...
    test "honours a custom --bus option" do
      project_with_robot()
      |> Igniter.compose_task("bb_ina219.install", ["--bus", "ftdi-3:17-i2c"])
      |> assert_has_patch("lib/test/application.ex", ~s'''
  ...
  describe "formatter" do
    test "imports bb_ina219 into .formatter.exs" do
      project_with_robot()
      |> Igniter.compose_task("bb_ina219.install")
      |> assert_has_patch(".formatter.exs", """
      + |  import_deps: [:bb_ina219, :bb]
      """)
    end
  end
  ...
  describe "notice" do
    test "prints a topology snippet for the user to paste" do
      project_with_robot()
      |> Igniter.compose_task("bb_ina219.install")
      |> assert_has_notice(&String.contains?(&1, "BB.INA219"))
    end
  end
